...
module diem_framework::transaction_fee {
    ...
    // fun burn_coin_fraction(coin: &mut Coin<LotusCoin>, burn_percentage: u8) acquires GasCoinCapabilities {
    //     assert!(burn_percentage <= 100, error::out_of_range(EINVALID_BURN_PERCENTAGE));
    ...
    public(friend) fun collect_fee(account: address, fee: u64) acquires CollectedFeesPerBlock {
        ...
        coin::collect_into_aggregatable_coin<LotusCoin>(account, fee, collected_amount);
    ...
    public fun user_pay_fee(sender: &signer, fee: Coin<LotusCoin>) acquires CollectedFeesPerBlock {
    ...
    public fun vm_pay_fee(sender: &signer, account: address, fee: Coin<LotusCoin>) acquires CollectedFeesPerBlock {
      // Need to track who is making payments to Fee Maker
    ...
    /// implementation
    fun pay_fee_impl(fee: Coin<LotusCoin>) acquires CollectedFeesPerBlock {
        ...
        // we burn them all at once. This way we avoid having a check for every transaction epilogue.
        let collected_amount = &mut collected_fees.amount;
        coin::merge_aggregatable_coin<LotusCoin>(collected_amount, fee);
    }
    ...
    // belt and suspenders
    public(friend) fun root_withdraw_all(root: &signer): Coin<LotusCoin> acquires CollectedFeesPerBlock {
      system_addresses::assert_ol(root);
      withdraw_all_impl(root)
    }

    // belt and suspenders implementation
    fun withdraw_all_impl(root: &signer): Coin<LotusCoin> acquires CollectedFeesPerBlock {
      system_addresses::assert_ol(root);

      let collected_fees = borrow_global_mut<CollectedFeesPerBlock>(@diem_framework);

      coin::drain_aggregatable_coin<LotusCoin>(&mut collected_fees.amount)
    }


    /// Only called during genesis.
    public(friend) fun store_diem_coin_burn_cap(diem_framework: &signer, burn_cap: BurnCapability<LotusCoin>) {
        system_addresses::assert_diem_framework(diem_framework);
        move_to(diem_framework, GasCoinCapabilities { burn_cap })
    }

    #[test_only]
    public fun test_root_withdraw_all(root: &signer): Coin<LotusCoin> acquires CollectedFeesPerBlock {
      system_addresses::assert_ol(root);
      withdraw_all_impl(root)
    }
    ...
    //     assert!(*option::borrow(&coin::supply<LotusCoin>()) == 0, 0);

    //     coin::destroy_zero(c1);
    //     let c2 = coin::mint<LotusCoin>(10, &mint_cap);
    //     assert!(*option::borrow(&coin::supply<LotusCoin>()) == 10, 0);
    ...
    // #[test(root = @lotus_framework, alice = @0xa11ce, bob = @0xb0b, carol = @0xca101)]
    ...
    // ) acquires GasCoinCapabilities, CollectedFeesPerBlock {
    //     //////// 0L ////////
    //     // changed to use LotusCoin for fees
    //     use std::signer;
    //     use lotus_framework::ol_account;
    //     use lotus_framework::libra_coin;
    ...
    //     assert!(*option::borrow(&coin::supply<LotusCoin>()) == 30000, 0);
    ...
    //     let collected_fees = borrow_global<CollectedFeesPerBlock>(@lotus_framework);
    //     assert!(coin::is_aggregatable_coin_zero(&collected_fees.amount), 0);
    ...
    //     // assert!(*option::borrow(&coin::supply<LotusCoin>()) == 29900, 0);
    ...
    //     // assert!(*option::borrow(&coin::supply<LotusCoin>()) == 29000, 0);
    ...
    //     assert!(coin::is_aggregatable_coin_zero(&collected_fees.amount), 0);
    //     assert!(*option::borrow(&collected_fees.proposer) == alice_addr, 0);
    //     // assert!(*option::borrow(&coin::supply<LotusCoin>()) == 28800, 0);
