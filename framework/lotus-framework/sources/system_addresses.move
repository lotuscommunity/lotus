module diem_framework::system_addresses {
    ...
    public fun is_lotus_framework_address(addr: address): bool {
        addr == @lotus_framework
    }

    public fun assert_ol(sig: &signer) {
      let addr = signer::address_of(sig);
        assert!(is_lotus_framework_address(addr) || is_reserved_address(addr) || is_core_resource_address(addr) || addr == @0x2,
        error::permission_denied(ENOT_OL_ROOT_ADDRESS))
    }

    public fun signer_is_ol_root(sig: &signer): bool {
      let addr = signer::address_of(sig);
      is_lotus_framework_address(addr) || is_reserved_address(addr) || is_core_resource_address(addr) || addr == @0x2
    }
}
