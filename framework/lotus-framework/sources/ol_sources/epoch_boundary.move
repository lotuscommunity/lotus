...
module diem_framework::epoch_boundary {
    ...
    use lotus_framework::musical_chairs;
    use lotus_framework::proof_of_fee;
    use lotus_framework::stake;
    use lotus_framework::lotus_coin::LotusCoin;
    use lotus_framework::rewards;
    ...
    use lotus_framework::infra_escrow;
    ...
    public(friend) fun initialize(framework_signer: &signer) {
      if (!exists<BoundaryStatus>(@lotus_framework)){
        move_to(framework_signer, reset());
      };

      // boundary bit can only be written by VM
      if (!exists<BoundaryBit>(@lotus_framework)) {
    ...
    u64) acquires BoundaryBit {
      ...
      let framework_signer = create_signer::create_signer(@lotus_framework);
      if (!exists<BoundaryBit>(@lotus_framework)) {
        ...
        let state = borrow_global_mut<BoundaryBit>(@lotus_framework);
    ...
    epoch_round: u64) acquires BoundaryStatus {
        ...
        let root = &create_signer::create_signer(@lotus_framework);
        let status = borrow_global_mut<BoundaryStatus>(@lotus_framework);
  ...
  public fun get_reconfig_success(): bool acquires BoundaryStatus {
    borrow_global<BoundaryStatus>(@lotus_framework).incoming_reconfig_success
  }
  #[view]
  public fun get_actual_vals(): vector<address> acquires BoundaryStatus {
    borrow_global<BoundaryStatus>(@lotus_framework).incoming_actual_vals
  }

  #[view]
  public fun get_qualified_bidders(): vector<address> acquires BoundaryStatus {
    borrow_global<BoundaryStatus>(@lotus_framework).incoming_only_qualified_bidders
  }

  #[view]
  public fun get_auction_winners(): vector<address> acquires BoundaryStatus {
    borrow_global<BoundaryStatus>(@lotus_framework).incoming_auction_winners
  }

  #[view]
  public fun get_seats_offered():u64 acquires BoundaryStatus {
    borrow_global<BoundaryStatus>(@lotus_framework).incoming_seats_offered
  }
