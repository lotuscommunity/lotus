spec lotus_framework::ol_account {
    spec module {
        pragma verify = true;
        // pragma aborts_if_is_strict;
        invariant [suspendable] chain_status::is_operating() ==> exists<lotus_coin::FinalMint>(@diem_framework);
        invariant [suspendable] chain_status::is_operating() ==>  exists<coin::CoinInfo<LotusCoin>>(@diem_framework);
    ...
    //     ensures exists<coin::CoinStore<LotusCoin>>(auth_key);
    // }
    ...
    spec withdraw(sender: &signer, amount: u64): Coin<LotusCoin>{
        include AssumeCoinRegistered;

        let account_addr = signer::address_of(sender);
        aborts_if amount == 0;

        let coin_store = global<coin::CoinStore<LotusCoin>>(account_addr);
        ...
        ensures result == Coin<LotusCoin>{value: amount};
    }

    spec schema AssumeCoinRegistered {
        sender: &signer;
        let account_addr = signer::address_of(sender);
        aborts_if !coin::is_account_registered<LotusCoin>(account_addr);
    }
    ...
    /// Check if the LotusCoin under the address existed.
    spec assert_account_is_registered_for_gas(addr: address) {
        aborts_if !account::exists_at(addr);
        aborts_if !coin::is_account_registered<LotusCoin>(addr);
    }
