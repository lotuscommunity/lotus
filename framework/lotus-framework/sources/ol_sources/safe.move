...
module lotus_framework::safe {
  ...
  use lotus_framework::system_addresses;
  ...
  #[test_only]
  friend lotus_framework::test_safe;
  ...
  public(friend) fun initialize(vm: &signer) {
   ...
   if (!exists<RootMultiSigRegistry>(@lotus_framework)) {
  ...
  fun add_to_registry(addr: address) acquires RootMultiSigRegistry {
    let reg = borrow_global_mut<RootMultiSigRegistry>(@lotus_framework);
  ...
  public(friend) fun root_security_fee_billing(vm: &signer): (u64, u64, bool) acquires RootMultiSigRegistry {
    system_addresses::assert_ol(vm);
    let reg = borrow_global<RootMultiSigRegistry>(@lotus_framework);
    ...
    while (i < vector::length(&reg.list)) {
      ...
      let fee = fixed_point32::multiply_u64(lotus_coin::balance(*multi_sig_addr), pct);
      ...
      if (option::is_some(&coin_opt)) {
        ...
        security_bill_amount = security_bill_amount + lotus_coin::value(&c);
