#[test_only]
module lotus_framework::test_governance {
  ...
  fun gov_threshold_can_resolve_early(root: &signer, alice: &signer, bob: &signer) {
    ...
    let early = voting::is_early_close_possible<GovernanceProposal>(@lotus_framework, prop_id);
    assert!(early, 73570004);

    // will it close
    let closed = voting::is_voting_closed<GovernanceProposal>(@lotus_framework, prop_id);
    ...
    let (can_resolve, _err) = voting::check_resolvable_ex_hash<GovernanceProposal>(@lotus_framework, prop_id);
