module lotus_framework::epoch_helper {
    ...
    public fun initialize(root: &signer) {
      if (!exists<EpochHelper>(@lotus_framework)){
    ...
    public(friend) fun set_epoch(epoch: u64) acquires EpochHelper{
      let state = borrow_global_mut<EpochHelper>(@lotus_framework);
      state.epoch = epoch;
    }

    #[view]
    public fun get_current_epoch():u64 acquires EpochHelper{
      let state = borrow_global<EpochHelper>(@lotus_framework);
      state.epoch
    }
}