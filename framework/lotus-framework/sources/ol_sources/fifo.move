...
module lotus_framework::fifo {
    ...
    fun empty<Element>(): FIFO<Element>{
    ...
    fun push<Element>(v: &mut FIFO<Element>, new_item: Element){
    ...
    fun push_LIFO<Element>(v: &mut FIFO<Element>, new_item: Element){
        vector::push_back<Element>(&mut v.outgoing, new_item);
    }

    /// grab the next element from the queue, removing it
    fun pop<Element>(v: &mut FIFO<Element>): Element{
    ...
    fun peek<Element>(v: &mut FIFO<Element>): & Element{
        perform_swap<Element>(v);
    ...
    fun peek_mut<Element>(v: &mut FIFO<Element>): &mut Element{
        perform_swap<Element>(v);
    ...
    fun len<Element>(v: & FIFO<Element>): u64{
