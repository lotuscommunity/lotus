...
module lotus_framework::fee_maker {
    ...
    public(friend) fun initialize(lotus_framework: &signer) {
      system_addresses::assert_ol(lotus_framework);
      if (!exists<EpochFeeMakerRegistry>(@lotus_framework)) {
        let registry = EpochFeeMakerRegistry {
          fee_makers: vector::empty(),
          epoch_fees_made: 0,
        };
        move_to(lotus_framework, registry);
      }
    }
    ...
    public(friend) fun epoch_reset_fee_maker(vm: &signer): bool acquires EpochFeeMakerRegistry, FeeMaker {
      ...
      let registry = borrow_global_mut<EpochFeeMakerRegistry>(@lotus_framework);
    ...
    public fun get_fee_makers(): vector<address> acquires EpochFeeMakerRegistry {
      let registry = borrow_global<EpochFeeMakerRegistry>(@lotus_framework);
      *&registry.fee_makers
    }
    ...
    public fun get_all_fees_made(): u64 acquires EpochFeeMakerRegistry {
      if (!exists<EpochFeeMakerRegistry>(@lotus_framework)) return 0;
