...
module lotus_framework::lotus_coin {
    use std::string;
    use std::error;
    use std::signer;
    use std::vector;
    ...
    final_supply: u64) acquires FinalMint {
      ...
      if (!exists<FinalMint>(@lotus_framework)) {
        move_to(diem_framework, FinalMint {
          value: final_supply
        });
      } else {
        let state = borrow_global_mut<FinalMint>(@lotus_framework);
    ...
    public fun get_final_supply(): u64 acquires FinalMint{
      borrow_global<FinalMint>(@lotus_framework).value
    }
