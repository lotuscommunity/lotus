...
module lotus_framework::proof_of_fee {
  use std::error;
  use std::signer;
  use std::vector;
  ...
  public(friend) fun init_genesis_baseline_reward(vm: &signer) {
    ...
    if (!exists<ConsensusReward>(@lotus_framework)) {
  ...
  ConsensusReward {
    ...
    let state = borrow_global_mut<ConsensusReward>(@lotus_framework);
  ...
  public(friend) fun fill_seats_and_get_price(
    ...
    let cr = borrow_global_mut<ConsensusReward>(@lotus_framework);
  ...
  public fun get_consensus_reward(): (u64, u64, u64, u64) acquires ConsensusReward {
    let b = borrow_global<ConsensusReward>(@lotus_framework);
