...
module lotus_framework::donor_voice_txs {
    use std::vector;
    use std::signer;
    use std::error;
    use std::guid;
    ...
    use lotus_framework::account::{Self, WithdrawCapability};
    use lotus_framework::donor_voice_governance;
    use lotus_framework::cumulative_deposits;
    use lotus_framework::transaction_fee;
    ...
    #[test_only]
    friend lotus_framework::test_donor_voice;
    #[test_only]
    friend lotus_framework::test_community_wallet;
  ...
  fun liquidation_handler(donor: &signer, multisig_address: address) acquires Freeze {
      ...
      if (option::is_some(&res) && *option::borrow(&res)) {
        ...
        let f = borrow_global_mut<Freeze>(multisig_address);
        f.is_frozen = true;
        // let f = borrow_global_mut<Registry>(@lotus_framework);
  ...
  public(friend) fun vm_liquidate(vm: &signer) acquires Freeze {
    system_addresses::assert_ol(vm);
    let liq = donor_voice::get_liquidation_queue(); //borrow_global_mut<Registry>(@lotus_framework);
    ...
    while (i < len) {
      ...
      if (option::is_some(&coin_opt)) {
        let c = option::extract(&mut coin_opt);
        transaction_fee::vm_pay_fee(vm, @lotus_framework, c);
      };
      option::destroy_none(coin_opt);
