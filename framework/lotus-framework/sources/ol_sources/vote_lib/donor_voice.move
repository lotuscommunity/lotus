...
module lotus_framework::donor_voice {
    use std::vector;
    use std::signer;
    use std::error;
    ...
    public fun is_root_init():bool {
      exists<Registry>(@lotus_framework)
    }
    ...
    fun add_to_registry(sig: &signer) acquires Registry {
      assert!(exists<Registry>(@lotus_framework), error::invalid_state(ENOT_REGISTRY_NOT_INIT));

      let addr = signer::address_of(sig);
      let list = get_root_registry();
      if (!vector::contains<address>(&list, &addr)) {
        let s = borrow_global_mut<Registry>(@lotus_framework);
    ...
    public(friend) fun add_to_liquidation_queue(addr: address) acquires Registry {
        let registry = borrow_global_mut<Registry>(@lotus_framework);
    ...
    public fun get_root_registry(): vector<address> acquires Registry{
      if (exists<Registry>(@lotus_framework)) {
        let s = borrow_global<Registry>(@lotus_framework);
        return *&s.list
      } else {
  ...
  public fun get_liquidation_queue(): vector<address> acquires Registry{
    let f = borrow_global<Registry>(@lotus_framework);
    *&f.liquidation_queue
  }
}
