...
module lotus_framework::infra_escrow{
    ...
    fun infra_pledge_withdraw(vm: &signer, amount: u64): Option<coin::Coin<LotusCoin>> {
    ...
    (bool, u64) {
        ...
        transaction_fee::vm_pay_fee(root, @lotus_framework, c); // don't attribute
        // to the user
        return(true, value)
    }
    ...
    fun user_pledge_infra(user_sig: &signer, amount: u64){
      pledge_accounts::user_pledge(user_sig, @lotus_framework, amount);
    }

    #[view]
    /// gets the amount a user has pledged to infra escrow
    public fun user_infra_pledge_balance(addr: address): u64 {
      pledge_accounts::get_user_pledge_amount(addr, @lotus_framework)
    }

    #[view]
    /// gets the amount a user has pledged to infra escrow
    public fun infra_escrow_balance(): u64 {
      pledge_accounts::get_available_to_beneficiary(@lotus_framework)
    }
