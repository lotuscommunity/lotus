module lotus_framework::recovery_mode {
  ...
  RecoveryMode {
    ...
    if (!exists<RecoveryMode>(@lotus_framework)){
      move_to(framework, RecoveryMode { on })
    } else {
      let state = borrow_global_mut<RecoveryMode>(@lotus_framework);
      state.on = on;
    }
  }
  ...
  public fun is_recovery_mode(): bool acquires RecoveryMode {
    borrow_global<RecoveryMode>(@lotus_framework).on
  }
