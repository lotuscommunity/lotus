spec diem_framework::transaction_fee {
    ...
    //         amount_to_burn;
    //     include (amount_to_burn > 0) ==> coin::AbortsIfNotExistCoinInfo<LotusCoin>;
    // }

    spec fun collectedFeesAggregator(): AggregatableCoin<LotusCoin> {
        ...
        requires
            (is_fees_collection_enabled() && option::is_some(maybe_supply)) ==>
                (aggregator::spec_aggregator_get_val(global<CollectedFeesPerBlock>(@diem_framework).amount.value) <=
                    optional_aggregator::optional_aggregator_value(option::spec_borrow(coin::get_coin_supply_opt<LotusCoin>())));
    }
    ...
    spec collect_fee(account: address, fee: u64) {
        ...
        aborts_if fee > 0 && !exists<coin::CoinStore<LotusCoin>>(account);
        aborts_if fee > 0 && global<coin::CoinStore<LotusCoin>>(account).coin.value < fee;
    ...
    spec store_diem_coin_burn_cap(diem_framework: &signer, burn_cap: BurnCapability<LotusCoin>) {
        use std::signer;
