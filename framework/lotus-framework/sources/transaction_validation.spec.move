spec diem_framework::transaction_validation {
    ...
    spec schema PrologueCommonAbortsIf {
        ...
        aborts_if !(txn_sequence_number == global<Account>(transaction_sender).sequence_number);
        aborts_if !exists<CoinStore<LotusCoin>>(transaction_sender);
        aborts_if !(global<CoinStore<LotusCoin>>(transaction_sender).coin.value >= max_transaction_fee);
    }
    ...
    spec epilogue(
        ...
        use diem_framework::coin::{CoinStore};
        use diem_framework::account::{Account};
        use diem_framework::lotus_coin::LotusCoin;
        ...
        aborts_if !exists<CoinStore<LotusCoin>>(addr);
        aborts_if !(global<CoinStore<LotusCoin>>(addr).coin.value >= transaction_fee_amount);

        aborts_if !exists<Account>(addr);
        aborts_if !(global<Account>(addr).sequence_number < MAX_U64);

        let pre_balance = global<coin::CoinStore<LotusCoin>>(addr).coin.value;
        let post balance = global<coin::CoinStore<LotusCoin>>(addr).coin.value;
