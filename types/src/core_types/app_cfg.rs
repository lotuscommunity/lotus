...
pub const CONFIG_FILE_NAME: &str = "lotus-cli-config.yaml";
...
impl AppCfg {
    ...
    pub fn load(file: Option<PathBuf>) -> anyhow::Result<Self> {
        ...
        if !path.exists() {
            bail!(format!(
                "lotus-cli-config.yaml dir does not exist at {}\nHave you initialized the configs with `lotus config init`?",
                path.to_str().unwrap()
            ))
        }
