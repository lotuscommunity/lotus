...
pub fn find_default_playlist(chain_id: Option<NamedChain>) -> anyhow::Result<Url> {
    let url: Url = match chain_id {
      ...
      "https://raw.githubusercontent.com/lotuscommunity/seed-peers/main/fullnode_seed_playlist_testnet.json"
      .parse()?
    },
      _ => { // MAINNET and everything else
          "https://raw.githubusercontent.com/lotuscommunity/seed-peers/main/fullnode_seed_playlist.json"
          .parse()?
      },
    };
    Ok(url)
